...
public interface PontoInteresseRepository extends JpaRepository<PontoInteresse, String> {
    ...
    @Query("SELECT p FROM PontoInteresse p WHERE " +
           ...
           "(p.cidadeEntidade IS NOT NULL AND (" +
           "  (:codigo IS NOT NULL AND LOWER(p.cidadeEntidade.codigo) = LOWER(:codigo)) OR " +
           "  (:nome IS NOT NULL AND LOWER(p.cidadeEntidade.nome) = LOWER(:nome))" +
           ")))")
